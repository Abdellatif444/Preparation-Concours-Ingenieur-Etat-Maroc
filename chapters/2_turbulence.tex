...
\begin{itemize}
    \item Le désordre et l'irrégularité de l'écoulement. \item Un caractère tridimensionnel et non-linéaire. \item La présence de tourbillons de différentes échelles avec un transfert d'énergie des plus grands vers les plus petits \cite{pope2000turbulent}. \end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{figures/figure2_1.png}
    \caption{Écoulement laminaire.}
    \label{fig:laminaire}
\end{figure}
...
Cette méthode permet de résoudre directement les équations de Navier-Stokes sans aucune modélisation. Elle présente ainsi l'avantage de donner accès à toutes les quantités instantanées considérées dans l'écoulement. Tous les mouvements doivent être résolus par cette méthode, depuis les plus petites échelles dissipatives jusqu'à l'échelle intégrale. Le nombre de mailles est alors important, ce qui a pour conséquences des temps de calcul extrêmement longs \cite{moin1998direct}. La capacité et la performance des calculateurs actuels ne cessent de progresser mais ne permettent pas encore de sonder des écoulements complexes dans un contexte industriel.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/figure2_3.png}
    \caption{DNS d'un jet d'air issu d'une tuyère.}
    \label{fig:dns_jet}
\end{figure}

\subsection{Simulation des grandes échelles (LES)}
\label{subsec:les}

Il s'agit d'une méthode numérique intermédiaire entre la DNS et les méthodes statistiques consistant à appliquer un filtre spatial en tout point du domaine \cite{lesieur2005large}. Le champ filtré est obtenu par un produit de convolution dans l'espace :
...
Cette approche consiste à mettre de côté le mouvement instantané du fluide, afin d'exprimer les équations du champ moyen en se servant de la décomposition de Reynolds des variables en une partie moyenne et une partie fluctuante : \cite{reynolds1895dynamical}
...
\begin{figure}[H]
    ...
    \caption{Décomposition des variables selon l'approche RANS.}
    \label{fig:decomposition_reynolds}
\end{figure}

En introduisant cette décomposition dans les équations de Navier-Stokes et en prenant la moyenne, on obtient les équations RANS (Reynolds-Averaged Navier-Stokes). Ce processus fait apparaître de nouvelles inconnues : les \textbf{tensions de Reynolds} (\( -\rho \overline{u_i' u_j'} \)), qui représentent l'influence des fluctuations turbulentes sur l'écoulement moyen \cite{hinze1975turbulence}.
...
Avec \( \mu_t \) la viscosité dynamique turbulente et \( k \) l'énergie cinétique de turbulence.
